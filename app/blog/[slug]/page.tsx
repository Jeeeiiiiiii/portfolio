...
export default async function BlogPostPage({ params }: { params: BlogPostParams }) {
  try {
    ...
    // Check if resolvedParams and params exist
    if (!resolvedParams || !resolvedParams.params || !resolvedParams.params.slug) {
      throw new Error('Slug is missing or invalid');
    }

    const { slug } = resolvedParams.params; // safely extract slug

    // Assuming getBlogPost is a function that fetches the blog post based on the slug
    const post = getBlogPost(slug); // fetch the blog post using the slug
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="flex gap-4">
            {/* Add share buttons as in your code */}
          </div>
        </div>
    ...
    return <div>Error fetching the post. Please try again later.</div>;
  }
}
